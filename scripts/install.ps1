...
function Get-LatestVersion {
    try {
        $latestRelease = Invoke-RestMethod -Uri "https://api.github.com/repos/dienlanhvietnam-lang/dmctn-mimo/releases/latest"
        return @{
...
Write-Host "Created by DMCTN`n" -ForegroundColor $Theme.Info

# Set TLS 1.2
[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12

# Main installation function
function Install-DmctnMimo {
    Write-Styled "Start downloading MiMo FREE" -Color $Theme.Primary -Prefix "Download"
    ...
    try {
        ...
        $asset = $releaseInfo.Assets | Where-Object { $_.name -eq "DmctnMimo_${version}_windows.exe" }
        if (!$asset) {
            Write-Styled "File not found: DmctnMimo_${version}_windows.exe" -Color $Theme.Error -Prefix "Error"
        ...
        $downloadPath = Join-Path $DownloadsPath "DmctnMimo_${version}_windows.exe"
