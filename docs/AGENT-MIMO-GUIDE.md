# MiMo FREE + MiMo CLI — hướng dẫn nội bộ cho Agent
...
|----------|-------|----------|
| **MiMo FREE** | `start.bat` → `main.py` | Python CLI: Cursor reset, MiMo login/slots, reset MiMo |
| **MiMo CLI** | `mimo/start.bat` | Terminal AI (`@mimo-ai/cli` / MiMoCode) |

Hai phần **độc lập** nhưng cùng repo. Config mới: `Documents\.dmctn-mimo\` (tự migrate từ `.mimo-vip` / `.mino-vip`).
...
**Env:** `DMCTN_MIMO_LANG`, `DMCTN_MIMO_KEEP_RUNNING`, `DMCTN_MIMO_QUIET`, `DMCTN_MIMO_LIVE_OAUTH=1` (chỉ E2E OAuth live, không CI).
...
**Optional:** `Documents\.dmctn-mimo\mimo_profile_hints.json` map `Profile 5 → gmail01@...` (label hint only).
